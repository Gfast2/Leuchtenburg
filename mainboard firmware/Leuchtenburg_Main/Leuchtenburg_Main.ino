...
//SOUND//-------------------------------------------------------------------
  ...
  volumnTrack(36, -5); //Lautstärke fuer alle Texte.
  /*
  for(int i = 30; i<36; i++)
    play(i);
  ...
  */
  for (int i = 1; i < 18; i++) // Stellen alle Gelesenetext wenige laut.
    volumnTrack(i, -13/*für vorne Waage-9*/); //Lautstark fuer alle lesen text.
  for (int i = 18; i < 21; i++) // Erfolg Sound
    volumnTrack(i, -5);
...
void loop()
{
  ...
  if ((NotAusStatus == 1) || (NotAusPressed == true)) {
    // in this metod there is no sound handling code. 
    // TODO: if we add sound handling, too.
    NotAusHandling();
  }
...
//------------------------------------------------------------------------------
    ...
    if (StartWert == 1) {       //Beim ersten Durchlauf kein G_Max Handling
      ...
      calibration();
      //Nach Errorhandling() auskommentiert und direkt calibration() eingehen.
      soundStatus_alt = 1;
      if (FirstStart == 1) {
        FirstStart = 0;
      }
...
//-----------Begin Statusabfrage (neue SollPositionen für Motoren)--------------
        ...
        // ERROR PRONE: VERY POSSIBLE: "GewichtSumme + Gewicht[4]" 
...
//SOUND//-------------------------------------------------------------------
          ...
          stop(36);
          //play(30);
          //soundAmb(1); //auskommentiert in 2016-3-2
          DoneFlag = 0;
          calibration();
          FaktorWarRichtig = false;
          delay(1000);
        ...
        if (Gewicht[4] > G_SchwellwertMin + 2) { // Falsche Zutate Handling
          ...
          if (G_Zustand == 3) {
            ...
            if (StatusNeu == 13) { //stand by Status
              ...
              //'5' stop all playing songs."
              // In other place applyied. 
              soundStatus = 0;
            }
            else {
